...
const authToken = "67ab6ebcc8ce9936309984877bec928e";
const client = require('twilio')(accountSid, authToken);

app.get('/send', (req, res) => {
    const bin_ip = req.body.bin_ip
    client.messages.create({
        body: `Your Dustbin with Bin_ip ${bin_ip} is FULL!`,
        from: '+15677043016',
        to: '+918516029236'
        })
...
const push_data = async ( client, moisture, ultraSonic) => {
    ...
    try{
        ...
        if(data.status === 200)
        {
            // console.log(data.data[0].bin_id);
            // nData.bin_id = data.data[0].bin_id
            // data.data[0].user_id ? nData.user_id = data.data[0].user_id : nData.user_id = ""
            const newdata = {
